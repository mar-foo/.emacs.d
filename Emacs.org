...
#+end_src
** Modeline
Use doom-modeline because it loads faster then [[https://github.com/TheBB/spaceline][spaceline]] and [[https://github.com/domtronn/spaceline-all-the-icons.el][spaceline-all-the-icons]]
#+begin_src emacs-lisp :tangle ~/.emacs.d/Emacs.el
...
#+begin_src emacs-lisp :tangle ~/.emacs.d/Emacs.el
  ...
  (use-package prescient
    ...
    :hook (icomplete-vertical-mode . prescient-persistent-mode))
#+end_src
...
#+begin_src emacs-lisp :tangle ~/.emacs.d/Emacs.el
  (use-package embark
    ...
    (evil-define-key '(normal insert visual emacs) 'global (kbd "C-,") 'embark-act)
    (let ((map minibuffer-local-completion-map))
...
#+begin_src emacs-lisp :tangle ~/.emacs.d/Emacs.el
  (use-package emms
    ...
    (evil-define-key 'normal emms-browser-map  "q" 'kill-current-buffer)
    :bind
    ("C-x C-/ /" . emms-browser)
    :general (mf/leader-keys
...
#+begin_src emacs-lisp :tangle ~/.emacs.d/Emacs.el
(setq switch-to-buffer-obey-display-actions t)  ;; Make switch-to-buffer obey display-buffer-alist rules (override for example EMMS' way to focus the browser buffer)
...
*ace-window* 
Completion framework: *orderless* *selectrum* *marginalia embark* *consult 
** From doom
